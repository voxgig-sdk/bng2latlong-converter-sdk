...
describe('CoordinateConversionDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new Bng2latlongConverterSDK({
  ...
  test('direct-load-coordinate_conversion', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-coordinate_conversion', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['BNG2LATLONG_CONVERTER_TEST_COORDINATE_CONVERSION_ENTID']
