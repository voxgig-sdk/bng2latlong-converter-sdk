...
//     (\`new Bng2latlongConverterSDK(...)\` / \`Bng2latlongConverterSDK.test(...)\` become
//     \`Bng2latlongConverterSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'Bng2latlongConverterSDK'
...
const TEST_SEED = {"entity":{"coordinate_conversion":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new Bng2latlongConverterSDK(...)\` and any
// \`Bng2latlongConverterSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+Bng2latlongConverterSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/Bng2latlongConverterSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
