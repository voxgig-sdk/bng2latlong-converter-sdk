...
module Bng2latlongConverterUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = Bng2latlongConverterResponse.new({})
    ...
    if fetch_err
      ...
      response = Bng2latlongConverterResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = Bng2latlongConverterResponse.new(fetched)
    else
