# Bng2latlongConverter Ruby SDK



The Ruby SDK for the Bng2latlongConverter API — an entity-oriented client using idiomatic Ruby conventions.

The SDK exposes the API as capitalised, semantic **Entities** — for example `client.CoordinateConversion` — with named operations (`load`) instead of raw URL paths and query strings. Working with resources and verbs keeps call sites self-describing and reduces cognitive load.
...
```ruby
client = Bng2latlongConverterSDK.test(testopts, sdkopts)
```
...
| `CoordinateConversion` | `(data) -> CoordinateConversionEntity` | Create a CoordinateConversion entity instance. |
...
raise a `Bng2latlongConverterError` (a `StandardError` subclass), so wrap
...
```
rb/
├── Bng2latlongConverter_sdk.rb       -- Main SDK module
...
The main module (`Bng2latlongConverter_sdk`) exports the SDK class
