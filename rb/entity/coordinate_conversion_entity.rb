...
class CoordinateConversionEntity
  ...
  def make
    opts = @_entopts.dup
    CoordinateConversionEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = Bng2latlongConverterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CoordinateConversion, Hash] the current CoordinateConversion data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = Bng2latlongConverterHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CoordinateConversion fields)
  def match_get
  ...
  #
  # @param reqmatch [CoordinateConversionLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.CoordinateConversion.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [CoordinateConversion, Hash] the loaded CoordinateConversion; raises Bng2latlongConverterError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = Bng2latlongConverterHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
