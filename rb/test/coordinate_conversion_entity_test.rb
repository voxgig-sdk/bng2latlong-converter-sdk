...
class CoordinateConversionEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set BNG2LATLONG_CONVERTER_TEST_COORDINATE_CONVERSION_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    coordinate_conversion_ref01_data_dt0_load_result = Helpers.to_map(coordinate_conversion_ref01_data_dt0_loaded.respond_to?(:data_get) ? coordinate_conversion_ref01_data_dt0_loaded.data_get : coordinate_conversion_ref01_data_dt0_loaded)
    assert !coordinate_conversion_ref01_data_dt0_load_result.nil?
    assert_equal coordinate_conversion_ref01_data_dt0_load_result["id"], coordinate_conversion_ref01_data["id"]
...
def coordinate_conversion_basic_setup(extra)
  ...
  entid_env_raw = ENV["BNG2LATLONG_CONVERTER_TEST_COORDINATE_CONVERSION_ENTID"]
