# bng2latlong-converter-cli

boru-driven command-line client **and** interactive REPL for the Bng2latlongConverter
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bng2latlong-converter-cli)
make build

# 2. See usage (words, entities, env vars)
./bng2latlong-converter-cli --help

# 3. Provide credentials once, via the environment
export BNG2LATLONG_CONVERTER_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./bng2latlong-converter-cli load 1 coordinate_conversion            # {id:1} shorthand
./bng2latlong-converter-cli load '{id:1}' coordinate_conversion       # explicit match map

# 5. Override the API base URL for a single call
BNG2LATLONG_CONVERTER_BASE=https://api.example.com ./bng2latlong-converter-cli load 1 coordinate_conversion

# 6. No arguments -> interactive REPL
./bng2latlong-converter-cli
bng2latlong-converter> load 1 coordinate_conversion
bng2latlong-converter> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bng2latlong-converter-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/bng2latlong-converter-cli load 1 coordinate_conversion
   ```
...
```sh
./bng2latlong-converter-cli load 1 coordinate_conversion          # scalar shorthand for {id:1}
./bng2latlong-converter-cli load '{id:1}' coordinate_conversion     # explicit match map
```
...
./bng2latlong-converter-cli load 1 coordinate_conversion
```
...
Run with no arguments to open a REPL (prompt `bng2latlong-converter>`). Each line is
evaluated as its own boru expression:

```text
$ ./bng2latlong-converter-cli
bng2latlong-converter> load 1 coordinate_conversion
bng2latlong-converter> /help
bng2latlong-converter> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `coordinate_conversion`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/bng2latlong-converter-cli`. |
...
(`./bng2latlong-converter-cli <expr>`) and interactively (the REPL), and expressions compose the
