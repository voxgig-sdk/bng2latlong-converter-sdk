...
// Bng2latlongConverter SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class Bng2latlongConverterTestFeature extends Bng2latlongConverterBaseFeature
{
    ...
    public function init(Bng2latlongConverterContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (Bng2latlongConverterContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = Bng2latlongConverterParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (Bng2latlongConverterContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(Bng2latlongConverterContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = Bng2latlongConverterParam::call($ctx, $k);
