...
// Bng2latlongConverter SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = Bng2latlongConverterOmni::makeStructRunner(
    ...
    {
        return Bng2latlongConverterOmni::objmodel(Bng2latlongConverterOmni::argval($val));
    }

    /** A single-case fixture value on the comparison side. */
    private static function outval($val)
    {
        return Bng2latlongConverterOmni::unmark($val);
    }

    /** A result normalised to the comparison model. */
    private static function resval($val)
    {
        return Bng2latlongConverterOmni::structfix($val, Struct::undef());
    }
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(Bng2latlongConverterOmni::structfix($now, Struct::undef()))
                        !== json_encode(Bng2latlongConverterOmni::structfix($val, Struct::undef()))) {
