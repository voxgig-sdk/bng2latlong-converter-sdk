...
//
// NewBng2latlongConverterSDK, not TestSDK: the `test` feature is transport: 'base' and
// REPLACES the transport, so a client in test mode would shadow the script.
func fcClient(kase map[string]any) *sdk.Bng2latlongConverterSDK {
	res, _ := kase["res"].([]any)
	opts := map[string]any{
	...
	return sdk.NewBng2latlongConverterSDK(opts)
}
...
func fcCandidates(client *sdk.Bng2latlongConverterSDK) []fcOp {
	out := []fcOp{}
...
func fcInvoke(client *sdk.Bng2latlongConverterSDK, op fcOp, ctrl map[string]any) error {
	cv := reflect.ValueOf(client)
	ct := cv.Type()
	mapType := reflect.TypeOf(map[string]any{})
...
// new go here.
func fcRecord(client *sdk.Bng2latlongConverterSDK, name string) any {
			...
			for _, raw := range cases {
				...
				for _, rawstep := range steps {
					...
					if code, ok := wanterr.(string); ok {
						...
						// the word.
						got := ""
						var sdkerr *sdk.Bng2latlongConverterError
