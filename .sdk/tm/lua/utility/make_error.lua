...
local function make_error_util(ctx, err)
  ...
  local msg = "Bng2latlongConverterSDK: " .. opname .. ": " .. errmsg
  ...
  local sdk_err = Bng2latlongConverterError.new("", msg, ctx)
  sdk_err.result = ctx.utility.clean(ctx, result)
  sdk_err.spec = ctx.utility.clean(ctx, spec)

  if type(err) == "table" and getmetatable(err) == Bng2latlongConverterError then
