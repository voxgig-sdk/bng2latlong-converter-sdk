...
class PrimaryUtilityTest < Minitest::Test
  ...
  def setup
    @runner = Bng2latlongConverterOmni.make_runner(TEST_JSON_FILE, Bng2latlongConverterSDK.test(nil, nil))
  ...
  def err_from_map(m)
    ...
    Bng2latlongConverterError.new(m["code"] || "", msg)
  end
  ...
  def test_make_point_basic
    ...
    subject = lambda do |ctx|
      ...
      err.nil? ? point : Bng2latlongConverterOmni.errify(err)
    end
  ...
  def test_operator_basic
    subject = lambda do |vin|
      vin = {} unless vin.is_a?(Hash)
      op = Bng2latlongConverterOperation.new(vin)
      {
        "entity" => op.entity,
        "name" => op.name,
        "input" => op.input,
