# bng2latlong-converter-mcp

[MCP](https://modelcontextprotocol.io) server exposing the Bng2latlongConverter SDK as
two agent tools — `bng2latlong-converter_list` and `bng2latlong-converter_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/bng2latlong-converter-mcp)
make build

# 2. Provide credentials via the environment
export BNG2LATLONG_CONVERTER_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user bng2latlong-converter \
  -- /absolute/path/to/bng2latlong-converter-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./bng2latlong-converter-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// bng2latlong-converter_list: first page of records
{ "entity": "coordinate_conversion" }
{ "entity": "coordinate_conversion", "query": { } }

// bng2latlong-converter_load: one record by id
{ "entity": "coordinate_conversion", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/bng2latlong-converter-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user bng2latlong-converter \
     -- "$PWD"/dist/*/bng2latlong-converter-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `bng2latlong-converter_list` and `bng2latlong-converter_load` tools now appear
   in new sessions. Ask the agent to *"list coordinate_conversion using bng2latlong-converter"*
   and it calls `bng2latlong-converter_list` with `{"entity":"coordinate_conversion"}`.
...
```sh
export BNG2LATLONG_CONVERTER_APIKEY=sk_live_xxx            # API key
export BNG2LATLONG_CONVERTER_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `bng2latlong-converter_list` tool
...
### Call the `bng2latlong-converter_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `bng2latlong-converter_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `bng2latlong-converter_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `BNG2LATLONG_CONVERTER_APIKEY` | API key sent with every request. |
| `BNG2LATLONG_CONVERTER_BASE` | Optional override of the API base URL. |
...
```sh
./bng2latlong-converter-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"bng2latlong-converter_load","arguments":{"entity":"coordinate_conversion","query":{"id":1}}}}'
```
