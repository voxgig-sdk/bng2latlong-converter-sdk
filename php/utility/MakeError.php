...
class Bng2latlongConverterMakeError
{
    public static function call(?Bng2latlongConverterContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new Bng2latlongConverterContext([], null);
        }
        $op = $ctx->op ?? new Bng2latlongConverterOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new Bng2latlongConverterResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof Bng2latlongConverterError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new Bng2latlongConverterError('', $msg, $ctx);
        $sdk_err->result = ($ctx->utility->clean)($ctx, $result);
        $sdk_err->spec = ($ctx->utility->clean)($ctx, $spec);
        if ($err instanceof Bng2latlongConverterError) {
