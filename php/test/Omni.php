...
//    only - a typed Bng2latlongConverterContext there would make every ctx
...
final class Bng2latlongConverterOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new Bng2latlongConverterContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new Bng2latlongConverterSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new Bng2latlongConverterResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new Bng2latlongConverterError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new Bng2latlongConverterResponse($clean['response']);
    ...
    public static function err_from_map($m): ?Bng2latlongConverterError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new Bng2latlongConverterError(is_string($code) ? $code : '', $msg);
    }
