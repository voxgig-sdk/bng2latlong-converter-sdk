...
import (
	...
	omni "github.com/voxgig-sdk/bng2latlong-converter-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.Bng2latlongConverterError); is {
				out := map[string]any{
					"name":    "Bng2latlongConverterError",
...
func omniCtx(arg any, client *sdk.Bng2latlongConverterSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.Bng2latlongConverterSDK); is {
			client = live
