...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BNG2LATLONG_CONVERTER_TEST_LIVE") == "TRUE" ||
		os.Getenv("BNG2LATLONG_CONVERTER_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BNG2LATLONG_CONVERTER_TEST_EXPLAIN"); explain != "" {
...
func makeCtxFromMap(ctxmap map[string]any, client *sdk.Bng2latlongConverterSDK, utility *sdk.Utility) *sdk.Context {
	if ctxmap == nil {
	...
	if resMap, ok := ctxmap["result"].(map[string]any); ok {
		...
		if errMap, ok := resMap["err"].(map[string]any); ok {
			if msg, ok := errMap["message"].(string); ok {
				ctx.Result.Err = &sdk.Bng2latlongConverterError{Msg: msg}
			}
		}
	}
...
func fixctx(ctx *sdk.Context, client *sdk.Bng2latlongConverterSDK) {
...
func errFromMap(m map[string]any) error {
	...
	return &sdk.Bng2latlongConverterError{Msg: msg, Code: code}
}
