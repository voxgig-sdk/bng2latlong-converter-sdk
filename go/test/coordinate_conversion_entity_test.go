...
func TestCoordinateConversionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		coordinateConversionRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.coordinate_conversion")))
		...
		coordinateConversionRef01MatchDt0 := map[string]any{
			"id": coordinateConversionRef01Data["id"],
		}
		coordinateConversionRef01DataDt0Loaded, err := coordinateConversionRef01Ent.Load(coordinateConversionRef01MatchDt0, nil)
		if err != nil {
			t.Fatalf("load failed: %v", err)
		}
		coordinateConversionRef01DataDt0LoadResult := core.ToMapAny(entityData(coordinateConversionRef01DataDt0Loaded))
		if coordinateConversionRef01DataDt0LoadResult == nil {
			t.Fatal("expected load result to be a map")
		}
		if coordinateConversionRef01DataDt0LoadResult["id"] != coordinateConversionRef01Data["id"] {
...
func coordinate_conversionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BNG2LATLONG_CONVERTER_TEST_COORDINATE_CONVERSION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BNG2LATLONG_CONVERTER_TEST_COORDINATE_CONVERSION_ENTID"])
