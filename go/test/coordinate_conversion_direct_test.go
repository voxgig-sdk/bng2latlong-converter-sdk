...
func TestCoordinateConversionDirect(t *testing.T) {
	t.Run("direct-load-coordinate_conversion", func(t *testing.T) {
		setup := coordinate_conversionDirectSetup(map[string]any{"id": "direct01"})
		_mode := "unit"
		if setup.live {
			_mode = "live"
		}
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-coordinate_conversion", _mode); _shouldSkip {
			if _reason == "" {
...
type coordinate_conversionDirectSetupResult struct {
	client *sdk.Bng2latlongConverterSDK
	calls  *[]map[string]any
	live   bool
	idmap  map[string]any
}

func coordinate_conversionDirectSetup(mockres any) *coordinate_conversionDirectSetupResult {
	loadEnvLocal()
	...
	if live {
		...
		return &coordinate_conversionDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &coordinate_conversionDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
