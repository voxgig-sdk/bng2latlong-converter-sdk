    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set BNG2LATLONG_CONVERTER_TEST_COORDINATE_CONVERSION_ENTID JSON to run live")
        client = setup["client"]
...
def _coordinate_conversion_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("BNG2LATLONG_CONVERTER_TEST_COORDINATE_CONVERSION_ENTID"))
    ...
    _live = env.get("BNG2LATLONG_CONVERTER_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("BNG2LATLONG_CONVERTER_TEST_EXPLAIN") == "TRUE",
        "live": _live,
