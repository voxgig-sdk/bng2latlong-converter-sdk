# Bng2latlongConverter SDK test runner
...
class Bng2latlongConverterTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            Bng2latlongConverterTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = Bng2latlongConverterTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = Bng2latlongConverterTestRunner.getenv("BNG2LATLONG_CONVERTER_TEST_LIVE")
        override = Bng2latlongConverterTestRunner.getenv("BNG2LATLONG_CONVERTER_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = Bng2latlongConverterTestRunner.getenv(key)
        ...
        explain = Bng2latlongConverterTestRunner.getenv("BNG2LATLONG_CONVERTER_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["BNG2LATLONG_CONVERTER_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if Bng2latlongConverterTestRunner._test_control is not None:
            return Bng2latlongConverterTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                Bng2latlongConverterTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            Bng2latlongConverterTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = Bng2latlongConverterTestRunner.load_test_control()
...
def load_env_local():
    Bng2latlongConverterTestRunner.load_env_local()


def env_override(m):
    return Bng2latlongConverterTestRunner.env_override(m)


def entity_data(v):
    return Bng2latlongConverterTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return Bng2latlongConverterTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return Bng2latlongConverterTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return Bng2latlongConverterTestRunner.load_test_control()


def live_client_options():
    return Bng2latlongConverterTestRunner.live_client_options()


def live_delay_ms():
    return Bng2latlongConverterTestRunner.live_delay_ms()
