...
class TestCoordinateConversionDirect:

    def test_should_direct_load_coordinate_conversion(self):
        setup = _coordinate_conversion_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-coordinate_conversion", "live" if setup["live"] else "unit")
        if _skip:
...
def _coordinate_conversion_direct_setup(mockres):
